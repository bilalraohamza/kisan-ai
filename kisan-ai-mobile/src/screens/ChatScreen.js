...
export default function ChatScreen() {
  ...
  const send = async () => {
    ...
    try {
      const farmProfile = JSON.parse(await AsyncStorage.getItem('farmProfile') || '{}');
...
const s = StyleSheet.create({
  header:      { backgroundColor: C.maroon, paddingHorizontal: 14, paddingBottom: 14 },
  ...
  inputBar:    { backgroundColor: C.white, borderTopWidth: 1, borderTopColor: C.sep, padding: 10, flexDirection: 'row', alignItems: 'flex-end', gap: 8 },
  input:       { flex: 1, backgroundColor: C.cream, borderRadius: 20, padding: 12, fontSize: 15, color: C.ink, borderWidth: 0.5, borderColor: C.sep, maxHeight: 100 },
  sendBtn:     { width: 44, height: 44, borderRadius: 22, backgroundColor: C.maroon, alignItems: 'center', justifyContent: 'center', borderWidth: 1.5, borderColor: C.gold },
});
